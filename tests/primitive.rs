...
fn test_int_encode_decode() {
    ...
    let _ = data_writer.write(3454i32);
    let _ = data_writer.write(567561i32);
    let _ = data_writer.commit_block();
	data_writer.flush_to_disk(datafile_name);
    assert_eq!(Ok("3454\n567561\n".to_string()), common::get_java_tool_output(datafile_name));
}
...
fn test_write_string() {
    ...
    data_writer.write("abcd".to_string()).unwrap();
    data_writer.write("efgh".to_string()).unwrap();
    let _ = data_writer.commit_block();
	data_writer.flush_to_disk(datafile_name);
    assert_eq!(Ok("\"abcd\"\n\"efgh\"\n".to_string()), common::get_java_tool_output(datafile_name));
}
...
fn test_write_bytes() {
    ...
    data_writer.write(b"ravro".to_vec()).unwrap();
    let _ = data_writer.commit_block();
	data_writer.flush_to_disk(datafile_name);
    assert_eq!(Ok("\"ravro\"\n".to_string()), common::get_java_tool_output(datafile_name));
}
...
fn test_write_long() {
	...
	data_writer.write(1i64).unwrap();
	data_writer.write(2i64).unwrap();
	data_writer.write(3i64).unwrap();
	data_writer.write(4i64).unwrap();
	data_writer.write(5i64).unwrap();
...
fn test_write_float() {
	...
	data_writer.write(54.254f32).unwrap();
	data_writer.write(7.325344f32).unwrap();
	let _ = data_writer.commit_block();
	data_writer.flush_to_disk(datafile_name);
	assert_eq!(Ok("54.254\n7.325344\n".to_string()), common::get_java_tool_output(datafile_name));
}
