...
fn test_write_map() {
	...
	let mut data_writer = DataWriter::new(map_schema, Codecs::Snappy).unwrap();
...
fn write_nested_record() {
	...
	let outer_rec = RecordSchema::new("dashboard_stats", None, vec![name_field, map_field,  Field::new_for_encoding("inner_rec", None, Schema::Record(inner_rec))]);
